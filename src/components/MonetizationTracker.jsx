...
export const MonetizationTracker = ({
  ...
  // If no deadline set, show a simpler progress view
  if (!deadline) {
      ...
      <div className="bg-gradient-to-br from-violet-900/40 to-purple-900/40 rounded-2xl p-6 border border-violet-700/50">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">From your analytics data</p>
            </div>
          </div>
          {current >= goal && (
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-2">
            <span className="text-gray-400">
              {formatMillions(current)} / {formatMillions(goal)} impressions
            </span>
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div className="bg-gray-800/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-white">{formatMillions(current)}</div>
          ...
          <div className="bg-gray-800/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-green-400">
              {formatMillions(Math.max(goal - current, 0))}
            </div>
        ...
        <div className="mt-4 pt-4 border-t border-violet-700/30">
          <div className="text-sm text-gray-400 mb-2">Progress Milestones</div>
          <div className="flex gap-2">
            {[20, 40, 60, 80, 100].map((milestone) => {
              const completed = progress >= milestone;
              return (
                <div
                  key={milestone}
                  ...
                  title={`${milestone}%: ${formatMillions(goal * milestone / 100)}`}
                />
              );
            })}
          </div>
  ...
  // With deadline - show full tracking
  const progress = goal > 0 ? Math.min((current / goal) * 100, 100) : 0;
  const daysLeft = daysUntil(deadline);
  const dailyNeeded = Math.ceil((goal - current) / Math.max(daysLeft, 1));

  // Calculate if on track (simplified - assumes 90 day goal period)
  const totalDays = 90;
  const daysPassed = Math.max(totalDays - daysLeft, 1);
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{formatMillions(goal)} impressions by deadline</p>
          </div>
        </div>
        <div
